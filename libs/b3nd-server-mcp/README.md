...
MCP (Model Context Protocol) transport — exposes a `Rig` as an MCP server over
stdio. Three tools matching `ProtocolInterfaceNode` exactly.
...
Returns a bare `@modelcontextprotocol/sdk` `Server` instance without connecting
a transport. Use this when you want to attach your own transport (e.g. SSE,
WebSocket) or compose the MCP server into a larger app.
...
The server also registers MCP resources from `rig.status().schema`. Each program
(e.g. `mutable`) becomes a readable resource at `b3nd://{program}`. Any
`b3nd://{uri}` resource URI is read directly from the rig.
