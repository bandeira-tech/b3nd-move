...
Optional transport packages for the B3nd framework. Both packages live
in this repo as independent JSR packages — install only the one you
need.
...
Both depend on [@bandeira-tech/b3nd-core](https://github.com/bandeira-tech/b3nd-core)
for `Rig`, `httpApi`, and the `ServerResolver` contract.

## Why a separate repo

Server transports drag in either Hono (HTTP) or HTTP/2 streaming code
(gRPC). Most consumers of `b3nd-core` only need the framework
foundation — types, encoding, clients, the rig, network primitives.
Keeping the server side here means `b3nd-core` stays small and
dependency-free.

## Usage

```typescript
import { Rig, createServers, MemoryStore, SimpleClient, connection } from "@bandeira-tech/b3nd-core";
